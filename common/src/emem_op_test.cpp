...
typedef struct _mem_allocator
{
	...
	MemPoolList mem_pool_lists[MAX_MEM_POOLS];
	ELock elock;
} mem_allocator;
...
MemAllocator MemAllocator_new()
{
	...
	for (; i < MAX_MEM_POOLS; i++) {
		ret.self->mem_pool_lists[i] = MemPoolList_new();
	}
	ELock_Init(&ret.self->elock);
	return ret;
}
void MemAllocator_delete(MemAllocator _self)
{
	...
	{
		MemPool pool =
		MemPoolList_pop_front(SELF.mem_pool_lists[i]);
		while (pool.self) {
            MemPool_delete(pool);
			pool = MemPoolList_pop_front(SELF.mem_pool_lists[i]);
		}
		MemPoolList_delete(SELF.mem_pool_lists[i]);
	}
	free(_self.self);
}
void* MemAllocator_alloc(MemAllocator _self, euint _size, bool _is_safe_alloc)
{
	...
	if (i < MAX_MEM_POOLS)
	{
		MemPoolList mpl = SELF.mem_pool_lists[i];
		MemPool mp = MemPoolList_pop_front(mpl);
		if (!mp.self) {
...
void MemAllocator_log(MemAllocator _self)
{
	...
	for (; i < MAX_MEM_POOLS; i++)
	{
		...
		MemPool mp = MemPoolList_pop_front(mpl);
		while (mp.self) {
			totel_refer_info info = MemPool_log(mp);
			ret.unused_mem_size += info.unused_mem_size;
			ret.used_mem_size += info.used_mem_size;
            mp = MemPoolList_pop_front(mpl);
		}
	}
#if BIT_WIDTH == 32
